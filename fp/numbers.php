...
<?php latin_web_getHeader(); ?>
		...
		<title>kgo.to / latin | FP: Number Words</title>
	</head>
	<body>
...
?>
	<script>
		...
		function fetchVerbBank() {
			return $.post('/latin/ajax/fetch', { start: 1, end: 40, randomized: true, reqSize: 800, type: 'nums'});
		}
		//from warmenhoven
		...
		function populateBox(word) {
			...
			$('#qbox .answerDetails').fadeTo(500, 0, function(){
				$('.answerDef', this).text(testAnswer);
			});
		...
		$(function() {
			...
			fetchVerbBank().done(function(msg) {
				wordbank = msg.result;
				promptNewWord();
			});
