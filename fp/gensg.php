...
<?php latin_web_getHeader(); ?>
		...
		<title>kgo.to / latin | FP: Genitive Singulars</title>
	</head>
	<body>
...
?>
	<script>
		...
		function fetchVerbBank(chp_min, chp_max) {
			return $.post('/latin//ajax/fetch', { start: chp_min, end: chp_max, randomized: true, reqSize: 800, type: 'gens'});
		}
		function answerWord() {
			//check guess
			//if right: 
		...
		function populateBox(word) {
			...
			$("#qbox .question .question-part").eq(1).attr('data-answer', pp[1]).text("____").addClass('desired animated infinite pulse animate-slow');
